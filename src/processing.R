...
for (lemma in c("alt-right")) {
  ...
  if (subsetting == "freq") {
    ...
    subs[["one"]][["cut"]] <- sub_cuts %>%
      filter(CUT == "one") %>%
      pull(DATE)
    subs[["two"]][["cut"]] <- sub_cuts %>%
      filter(CUT == "two") %>%
      pull(DATE)
    subs[["three"]][["cut"]] <- sub_cuts %>%
      filter(CUT == "three") %>%
      pull(DATE)
    subs[["four"]][["cut"]] <- sub_cuts %>%
      filter(CUT == "four") %>%
      pull(DATE)
    subs[["full"]][["cut"]] <- "NA"
    ...
    subs[["one"]][["start"]] <- sub_limits %>%
      filter(SUB == "one", LIMIT == "start") %>%
      pull(DATE)
    subs[["one"]][["end"]] <- sub_limits %>%
      filter(SUB == "one", LIMIT == "end") %>%
      pull(DATE)
    subs[["two"]][["start"]] <- sub_limits %>%
      filter(SUB == "two", LIMIT == "start") %>%
      pull(DATE)
    subs[["two"]][["end"]] <- sub_limits %>%
      filter(SUB == "two", LIMIT == "end") %>%
      pull(DATE)
    subs[["three"]][["start"]] <- sub_limits %>%
      filter(SUB == "three", LIMIT == "start") %>%
      pull(DATE)
    subs[["three"]][["end"]] <- sub_limits %>%
      filter(SUB == "three", LIMIT == "end") %>%
      pull(DATE)
    subs[["four"]][["start"]] <- sub_limits %>%
      filter(SUB == "four", LIMIT == "start") %>%
      pull(DATE)
    subs[["four"]][["end"]] <- sub_limits %>%
      filter(SUB == "four", LIMIT == "end") %>%
      pull(DATE)
    subs[["full"]][["start"]] <- sub_limits %>%
      filter(SUB == "full", LIMIT == "start") %>%
      pull(DATE)
    subs[["full"]][["end"]] <- sub_limits %>%
      filter(SUB == "full", LIMIT == "end") %>%
      pull(DATE)
