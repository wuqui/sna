...
df_comp %<>% 
  filter(
    ...
    # established words
    !LEMMA %in% c('climate crisis', 'global heating', 'greenwashing', 'political correctness', 'refugee crisis', 'artificial intelligence', 'yellow', 'Anglo-Saxon', 'fidget spinner', 'cookprocessor')
    # successful words: USES_TOT >= 10000
    # unsuccessful words: USES_TOT <= 10000
    # outliers: big dick energy
  )
```
...
df_comp %>% 
  ...
  mutate(EDGES_NODE = EDGES / NODES) %>%
  select(SUBSET, NODES, EDGES, EDGES_NODE) %>%
  arrange((SUBSET))
```
...
df_comp %>% 
  filter(
    ...
    LEMMA %in% cases) %>% 
  mutate(EDGES_NODE = EDGES / NODES) %>%
  select(LEMMA, NODES, EDGES, EDGES_NODE) %>%
  arrange(desc(EDGES_NODE))
```
...
#### check one variable vs. all others
...
```{r}
cor.test(df_corr$CENT_DEGREE_LOG_CENT, df_corr$USES_LOG_CENT, 
         method='spearman')
```
...
```{r}
cor.test(df_corr$CENT_DEGREE_LOG_CENT, df_corr$AGE_LOG_CENT,
         method='spearman')
```
...
```{r}
cor.test(df_corr$CENT_DEGREE_LOG_CENT, df_corr$COEF_VAR_LOG_CENT,
         method='spearman')
```
...
### predicting total usage frequency

#### from `CENT_DEGREE` in first subset
...
df_comp %>%
  filter(SUBSET == 'one') %>%
  do(tidy((cor.test(.$USES_LOG_CENT, .$USES_TOT_LOG_CENT,  method='spearman'))))
```


### prediciting uses in last subset
...
cor.test(splt[[1]][1:99, ]$CENT_DEGREE_LOG_CENT, splt[[2]][1:99, ]$USES_LOG_CENT)
```
