...
\section{Results}
  ...
  The degree of diffusion of new words is commonly approximated by a how many times speakers have used these words in a corpus. The most fundamental way of using this information is to cumulate usage counts and to rely on the total number of uses observed. In the following, I will use this most fundamental measure of diffusion as a baseline before I zoom in to get a more differentiated picture of the temporal and social dynamics of diffusion.
  ...
  \todo{selection criteria for cases}
  % typical examples, dynamics in usage intensity and social characteristics of diffusion
...
\todo{rationale: cumulated potential exposure}

  Neologisms that show the highest total frequency counts such as \emph{smartwatch} can plausibly be assumed to be familiar to many speakers. It seems unlikely that its roughly 1 Mio. active uses stem exclusively from only few, tight-knit communities of users (e.g. techno-enthusiasts) and it seems likely that big parts of the speech community have been exposed to the term.
...
While the end points of the graph show the total frequency counts as shown in Table~\ref{subtab:freq-total-cases}, the offsets and slopes of the trajectories of usage intensity additionally reveal differences in the temporal dynamics between lexemes.



starting points

  The selected cases differ regarding the starting points of their diffusion. While the oldest new word, \emph{hyperlocal}, started to spread in 2009, \emph{alt-left} is the newest candidate and enters the scene with a late, but very steep increase in 2016.
...
slopes
  ...
  drawback: dynamics not captured
    ...
    lack of further uses: \emph{alt-left}
