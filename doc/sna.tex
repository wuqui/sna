  ...
  \marginpar{total usage frequency will be used to evaluate the social network metrics}
    ...
    \subsubsection{Coefficient of variation}
      ...
      \subsubsection{Summary of frequency-based measures}
        ...
        For example, \ol{hyperlocal}, \ol{solopreneur}, and \ol{alt-left}, are all in the medium range of neologisms if grouped by total usage frequency as in Table~\ref{subtab:freq-total-cases}. Taking this crudest measure as an indicator of degrees of diffusion, it would seem that these words are roughly equally conventional among users on Twitter.

        However, the picture changes considerably once we are taking into account the temporal dynamics underlying their use.

        \marginpar{trend} The cumulative increase in uses over time presented in Figure~\ref{fig:freq_cum_cases} indicate a consistent linear trend for the use of \ol{hyperlocal}, a rapid surge of diffusion followed by a slightly negative, concave trend in its later stages for \ol{solopreneur}. Featuring an even more extreme spurt of diffusion, the use of \ol{alt-left} explodes in August 2017, yet its popularity quickly implodes soon after, and it seems to attract very little popularity among the majority of speakers after that.
