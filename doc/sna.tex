...
\section{Modeling and measuring the diffusion of lexical innovations}
  ...
  \subsection{Measuring diffusion}
    ...
    \subsubsection{Previous empirical approaches to the diffusion of lexical innovations}
      ...
      \begin{qitem}
        # case studies \parencite{Hohenhaus2006BouncebackabilityWebascorpusbased}
        # specific domains \parencite{Elsen2004Neologismen}
        # web corpora for neologism research: \parencite{Renouf2006WebCorpIntegrated, Kerremans2012NeoCrawlerIdentifying,LemnitzerWortwarte,Gerard2017LogoscopeSemiautomatic,Cartier2017NeoveilleWeb}
        # social media corpora
          ## Twitter: \cite{Grieve2016AnalyzingLexical,Eisenstein2014DiffusionLexical}
          ## Reddit
      \end{qitem} 
      
    \subsubsection{Frequency as a measure of diffusion}
      ...
      \begin{qitem}
        ...
        # Therefore,
          ## I will use Social Network Analysis for a more direct operationalisation of social diffusion
          ## use frequency measures as a baseline
      \end{qitem}
...
\section{Discussion}
  ...
  \subsection{Temporal dynamics of diffusion}
    ...
    Secondly, it was shown that the age of neologisms provides important information about their diffusion processes. Neologisms such as \ol{hyperlocal} and \ol{alt-left}, which are comparable in total use frequency, but differ strongly with regard to their observed lifespan in the corpus, show different pathways and degrees of diffusion. Older neologisms typically show higher degrees of diffusion when their use is distributed more evenly across longer periods of consistent usage (\ol{hyperlocal}), than younger neologisms whose use goes almost exclusively back to a short period of highly intensive use (\ol{alt-left}). This positive relationship between the age of neologisms and their degrees of diffusion was supported by the significant correlation with centrality in the network analysis. While a longitudinal, predictive approach to the fate of lexical innovations is beyond the scope of the present paper, it seems possible that neologisms exhibit the Lindy Effect: the longer new words remain in use by the speech community, the less likely they might be to become obsolete. Again, the fate of new words ultimately depends on the conceptual salience of the objects and practices they denote: whether \ol{smartwatch} and \ol{blockchain} outlive previous neologisms such as \ol{Walkman} and \ol{Discman} ultimately depends on the future success of these products in our society.

    Lastly, temporal volatility is an important factor in the diffusion of neologisms. While some candidates show fairly consistent usage frequency over time (e.g. \ol{hyperlocal}, \ol{upskill}), most exhibit considerable fluctuations. For some words in the sample, recurrent spikes in usage intensity are an inherent part of their usage profile. The neologisms \ol{youthquake} is characterised by spikes in usage intensity when relevant to current public affairs, but shows low frequency of use in the intermediate intervals. Due to the nature of this behaviour, this pattern has been termed \enquote{topical} by previous work~\parencite{Fischer1998LexicalChange}. Cases such as \ol{poppygate}, for which these topical spikes occur in fairly regular, periodic intervals have been classified as \enquote{recurrent semi-conventionalization}~\parencite{Kerremans2015WebNew}. In both cases, total frequency counts cannot provide an accurate estimation of degrees of diffusion since they lack information about such temporal patterns which are central to these cases of lexical innovation. The network approach to diffusion in Section~\ref{subsec:sna} revealed a negative correlation between volatility and degrees of diffusion. It seems that neologisms that are used less consistently over time are less likely to reach advanced degrees of diffusion. Moreover, comparing frequency counts and degree centrality suggested that frequency tends to overestimate the degree of diffusion of topical neologisms. This is in line with the observation that isolated spikes in usage intensity tend to go back to disproportionally smaller parts of the speech community.

    \marginpar{\textsc{trend} + \textsc{volatility} $\neq$ S-curve}
  ...
  \subsection{Social dynamics of diffusion}
    ...
    Firstly, linguistic \emph{innovations} are by definition new and not (yet) conventional among the speech community. It is therefore to be expected that their use is unevenly distributed across communities of speakers. Since frequency counts alone do not provide information about this distribution, sociolinguistic data are needed to assess the degrees of diffusion of linguistic innovations.  

    Secondly, unlike linguistic innovations in other domains such as morphology or syntax, \emph{lexical} innovations are often consciously coined and have a very specific communicative function. Their usefulness is closely tied to the conceptual salience of the entity they denote. The semantic carrying capacity of new words is thus much more likely to exhibit social and temporal variation than the functional potential of grammatical constructions. While speakers of English from all walks of life have always felt the urge to talk about the future, the urge to talk about the future of \enquote{blockchain} has only come up very recently, is (still) limited to specific parts of the speech community, and might not persist in the future. In other words, the use of lexical innovations exhibits greater social and temporal variation than innovations in other linguistic domains. The interpretation of aggregated frequency counts, which suggest a uniform distribution of use across time and across the speech community, is thus particularly problematic for assessing the diffusion of new words.

    Moreover, neologisms typically arise in specific communities of practice and often show, at least initially, high degrees of social indexicality with regard to these communities. The present dataset includes several examples which are associated with youth language (\ol{fleek}, \ol{lituation}) and political discourse (\ol{birther}, \ol{alt-left}). A term like \ol{alt-left}, which could in principle be used neutrally to designate the political far-left, is highly socially indexical of the far-right community it emerged from. Therefore it is less likely to be used by speakers outside this community, unless they are willing to be associated with it. Neologisms which are socially indexical are thus more community-specific. Even when speakers outside this community are familiar with these terms, they are less likely to use them. Usage frequency counts miss such effects, since they only capture active uses of neologisms.

    \marginpar{Den Teil ab hier mach ich gerade fertig.}
    
    \begin{qitem}
      # frequency has several drawbacks when it comes to investigating social variation
      # social network analysis can complement frequency measures
      # while this is particularly important for linguistic innovation on the level of the lexicon, it also applies to other linguistic domains; any time one aims to assess the conventionality of linguistic constructions
      # this has big potential for (computational) sociolinguistics 
        ## it is a powerful tool for sociolinguistic research (on online social networks, which are more and more important)
        ## it can extend previous and current sociolinguistic research; e.g. research on geographical variation; mentioned as desideratum in \cite{Grieve2019MappingLexical}
        ## sociolinguists should \enquote{upskill} and use SNA more
    \end{qitem}

      \marginpar{S-curve model: early adopters, influencers, density, weak ties}

  \subsection{Future work}
  
    \marginpar{could potentially be part of the conclusion}
    ...
    \begin{qitem}
      ...
      # investigate other dimensions of diffusion \parencite{Schmid2020DynamicsLinguistic}
        # across contexts: different web registers~\parencite{Biber2016RegisterVariation}
