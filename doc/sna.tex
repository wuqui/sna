  ...
  \marginpar{total usage frequency will be used to evaluate the social network metrics}
    ...
    \subsubsection{Absolute frequency}
      ...
      \qpar{Volatility}
        ...
        The absolute counts provide a more detailed picture of temporal dynamics of use, however. While the cumulative counts in Figure \ref{fig:freq_cum_cases} suggest smooth trajectories, the plots in Figure \ref{fig:freq-abs} indicate that the selected cases differ significantly in terms of the consistency with which they are used in the corpus.

        gradual increase

          The neologisms \ol{upskill} shows the smoothest trajectory of diffusion among the candidate neologisms. Aside from two smaller spikes, at the end of 2016 and 2018, it has steadily increased in its use since its first attestation in the corpus at the end of 2007. Neither its frequency counts, nor the corpus data suggest that its diffusion was triggered or propagated by topical events or the determining impact of influential users or user groups. After a long period of very slow, but consistent increase in frequency, its diffusion has accelerated in recent years. While its future remains uncertain, its previous trajectory resembles most closely the earlier phases of spread as predicted by S-curve models.
        ...
        volatile
          \ol{solopreneur}
            less stable than \ol{hyperlocal}
            impact of influencers and books
            popularity tends to decline

          \ol{alt-left} and \ol{alt-right}
            \ol{alt-right}
              older
              stable in the beginning and end
              massive spike
            \ol{alt-left}
              younger
              same spike
        ...
        topical: \ol{poppygate}
          no gradual trajectory of diffusion
          consistent use in the form of recurrent spikes
            \enquote{topicality}~\parencite{Fischer1998LexicalChange}
            \enquote{recurrent semi-conventionalization}~\parencite{Kerremans2015WebNew})

    \subsection{Coefficient of variation}

  --> capture this quantitatively
  --> extend analysis to full sample

  cases: confirms observations
  full sample: seems plausible
    most volatile
    least volatile
