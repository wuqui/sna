  ...
  \marginpar{total usage frequency will be used to evaluate the social network metrics}
    ...
    \subsubsection{Coefficient of variation}
      ...
      \subsubsection{Summary of frequency-based measures}
        ...
        So far, I have used frequency-based visualizations and metrics to assess the degrees and pathways of diffusion of neologisms.

        In a first step, I used the most common measure for assessing the conventionality of new words: their total frequency of occurrence in the corpus. In the following steps, I extended the frequency-based approach by including temporal information in the analysis. Zooming in on temporal dynamics of use surfaced different pathways of diffusion. Notably, it revealed substantial differences in the diachronic usage profiles of neologisms with comparable total frequency counts.

        \marginpar{total frequency} Within the group of selected cases, \ol{hyperlocal}, \ol{solopreneur}, and \ol{alt-left}, for example, would all be placed in the medium range of the conventionality continuum if grouped by total usage frequency alone, as presented in Table~\ref{subtab:freq-total-cases}. Taking this crudest measure as an indicator of degrees of diffusion, it would seem that these words are roughly equally conventional among users on Twitter.

        However, adding the temporal dynamics of diffusion to the picture reveals significant differences between their diachronic usage profiles, which seem important for assessing their pathways and degrees of diffusion.

        \marginpar{hyperlocal}
        ...
        and presents an initial period of accelerated diffusion followed by an extended stable level of consistent use over the last five years of its observed lifespan. This consistency is further corroborated by its low coefficient of variation (Table~\ref{subtab:coef-var-cases}). In sum, the balanced nature of this frequency-based usage profile suggests a relatively organic trajectory of diffusion, culminating in a fairly advanced degree of conventionality in the recent past. The fact that \ol{hyperlocal} was added to the OED in 2015 supports these observations and assumptions.

        \marginpar{solopreneur}

          By comparison, \ol{solopreneur},
