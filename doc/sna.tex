...
\section{Discussion}
  ...
  \subsection{Social dynamics of diffusion}
    ...
    These discrepancies in results are in accordance with the differences in the way both approaches approach the assessment of diffusion. Successful diffusion of neologisms was defined as spread to new speakers and new communities.

    Using the frequency of occurrence of a neologism in a corpus to approximate to which degree it is familiar to bigger parts of the speech community relies on several assumptions~\parencite{Stefanowitsch2017CorpusbasedPerspective} which might only be accurate to certain extent.
    ...
    Secondly, usage frequency only captures active uses of the term and is blind to the number of speakers who are familiar with the term, but have not used it in the corpus. By contrast, social network metrics also include speakers who have only been passively exposed to the term, and thus covers a broader, and arguably more relevant definition of \enquote{familiarity}, which avoids having to rely on the assumption that the observed output of speakers in the corpus is representative of the input to speakers in the speech community~\cite{Stefanowitsch2017CorpusbasedPerspective}.

    Lastly, the number of uses observed might not be indicative of whether a neologism has spread beyond certain sub-communities and has reached a broader spectrum of the speech community.

    Figure~\ref{subfig:cent-vs-freq_cases} presented highlighted several cases, for which centrality indicated significantly lower degrees of diffusion than frequency. 

    The social networks of \ol{alt-left} demonstrated a case of political fragmentation and polarization~\parencite{Sunstein2018RepublicDivided} in the community of speakers, 



    that its use goes back almost exclusively to a well-connected community of like-minded individuals. While its network of users 


    number of nodes in \ref{fig:net_last_cases} 
    ties
      \ol{alt-left}
      \ol{hyperlocal}
    ...
    particularly important for \emph{lexical} \emph{innovations}
      ...
      bound to cultural conceptual salience (variable \enquote{semantic carrying capacity}~\parencite{Nini2017ApplicationGrowth})
        ...
        not universal across communities
